...
namespace App\Domain\Shortage\Support;

/**
 * Rounding money, once — Shortages' own copy.
 *
 * A fifth copy beside Order, Inventory, PurchaseOrder and Investor, and deliberately so: RULES §3
 * forbids one context importing another's helper, and unifying the five into `App\Support` is an
 * existing item in the backlog that must not ride in on a feature branch.
 *
 * bcmath **truncates** rather than rounds, so `bcadd('1.005', '0', 2)` is 1.00 — here that would
 * be a purchase recorded a fraction under what was handed over, on every entry. Adding half a
 * minor unit before cutting gives round-half-away-from-zero.
 *
